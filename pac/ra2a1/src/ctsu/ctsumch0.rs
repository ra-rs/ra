...
#[doc = "Field `CTSUMCH0` reader - CTSU Measurement Channel 0.Note1: Writing to these bits is enabled only in self-capacitance single scan mode (CTSUCR1.CTSUMD\\[1:0\\]
...
#[doc = "CTSU Measurement Channel 0.Note1: Writing to these bits is enabled only in self-capacitance single scan mode (CTSUCR1.CTSUMD\\[1:0\\]
bits = 00b).Note2: If the value of CTSUMCH0 was set to b'111111 in mode other than self-capacitor single scan mode, the measurement is stopped.\n\nValue on reset: 63"]
...
#[doc = "Field `CTSUMCH0` writer - CTSU Measurement Channel 0.Note1: Writing to these bits is enabled only in self-capacitance single scan mode (CTSUCR1.CTSUMD\\[1:0\\]
bits = 00b).Note2: If the value of CTSUMCH0 was set to b'111111 in mode other than self-capacitor single scan mode, the measurement is stopped."]
pub type CTSUMCH0_W<'a, const O: u8> =
    crate::FieldWriter<'a, u8, CTSUMCH0_SPEC, u8, CTSUMCH0_A, 6, O>;
impl R {
    #[doc = "Bits 0:5 - CTSU Measurement Channel 0.Note1: Writing to these bits is enabled only in self-capacitance single scan mode (CTSUCR1.CTSUMD\\[1:0\\]
bits = 00b).Note2: If the value of CTSUMCH0 was set to b'111111 in mode other than self-capacitor single scan mode, the measurement is stopped."]
    #[inline(always)]
