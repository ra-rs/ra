...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum AADJP_A {
    #[doc = "0: In normal operation mode, add or subtract the RADJ.ADJ\\[5:0\\]
bits from the prescaler count value every 32 seconds. In low-consumption clock mode, add or subtract the RADJ.ADJ\\[5:0\\]
bits from the 64-Hz counter count value every 8192 seconds."]
    _0 = 0,
    #[doc = "1: In normal operation mode, add or subtract the RADJ.ADJ\\[5:0\\]
bits from the prescaler count value every 8 seconds. In low-consumption clock mode, add or subtract the RADJ.ADJ\\[5:0\\]
bits from the 64-Hz counter count value every 2048 seconds."]
    _1 = 1,
}
...
impl<'a, const O: u8> AADJP_W<'a, O> {
    #[doc = "In normal operation mode, add or subtract the RADJ.ADJ\\[5:0\\]
bits from the prescaler count value every 32 seconds. In low-consumption clock mode, add or subtract the RADJ.ADJ\\[5:0\\]
bits from the 64-Hz counter count value every 8192 seconds."]
    #[inline(always)]
    pub fn _0(self) -> &'a mut W {
