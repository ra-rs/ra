...
#[repr(u8)]
pub enum TOMDS_A {
    #[doc = "0: Timeout is detected during the following conditions: The bus is busy (BCST.BFREF = 0) in master mode.IICâ\u{80}\u{99}s own slave address is detected and the bus is busy in slave mode.The bus is free (BCST.BFREF = 1) while generation of a START condition is requested (CNDCTL.STCND = 1)."]
    _00 = 0,
...
impl<'a, const O: u8> TOMDS_W<'a, O> {
    #[doc = "Timeout is detected during the following conditions: The bus is busy (BCST.BFREF = 0) in master mode.IICâ\u{80}\u{99}s own slave address is detected and the bus is busy in slave mode.The bus is free (BCST.BFREF = 1) while generation of a START condition is requested (CNDCTL.STCND = 1)."]
    #[inline(always)]
    pub fn _00(self) -> &'a mut W {
